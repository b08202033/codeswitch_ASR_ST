...
def get_args_parser():
    ...
    parser.add_argument('--model_name_or_path', '-m', type=str, default="openai/whisper-large-v3") # MediaTek-Research/Clairaudience
    ...
    parser.add_argument('--dataset_path', '-d', type=str, default='./zh_en') # ky552/ML2021_ASR, ky552/cszs_fr_en
...
def main(args):
    ...
    def Clairaudience_collate_fn(batch):
        file = []
        for item in batch:
            if 'label' in dataset.features.keys():
                label = 'correct' if item['label'] == 0 else 'wrong'
        ...
        if example is not None:
            input_feature = torch.stack([whisper_feature_extractor(np.hstack((example['audio']['array'], item['audio']["array"])).astype('float32')) for item in batch])
        else:
            input_feature = torch.stack([whisper_feature_extractor(item['audio']["array"].astype('float32')) for item in batch])
    ...
    dataloader = DataLoader(
        ...
        collate_fn = Clairaudience_collate_fn if args.model_name_or_path == "MediaTek-Research/Clairaudience" else collate_fn,
    )
    ...
    if args.model_name_or_path == "MediaTek-Research/Clairaudience":
    ...
    for b in tqdm(dataloader):
        ...
        for file, t, p in zip(b["file"], b["transcription"], predictions):
            ...
            with open('fr_en_no_prompt_tmp.txt', 'a') as f:
                f.write(f"{file} ||| {t} ||| {p}\n")
